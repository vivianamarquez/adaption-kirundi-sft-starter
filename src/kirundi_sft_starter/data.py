...
def load_kirnews_prompts(config: dict[str, Any]) -> pd.DataFrame:
    eval_cfg = config["datasets"]["kirnews"]
    labels = config.get("evaluation_labels") or [
        "politics",
        "sport",
        "economy",
        "health",
    ...
    ds = ds.shuffle(seed=config["project"]["random_seed"])
    ds = ds.select(range(min(eval_cfg["sample_size"], len(ds))))

    rows = []
    label_names = ds.features["label"].names if "label" in ds.features else labels
    label_text = ", ".join(labels)
    for i, row in enumerate(ds):
        gold = row.get("en_label") or label_names[int(row["label"])]
        article = f"{row.get('title', '')}\n\n{row.get('content', '')}".strip()
        prompt = (
            "Classify this Kirundi article into one of the following English labels: "
            f"{label_text}. Return only the English label.\n\nArticle:\n{article}"
        )
        rows.append(
            {
                "prompt_id": f"kirnews_{i:05d}",
                "prompt": prompt,
                "gold_label": gold,
                "title": row.get("title", ""),
